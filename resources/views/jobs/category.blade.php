...
@section('title', $category->name . ' Jobs - Find Opportunities')
    ...
    <main class="max-w-7xl mx-auto px-6 py-12">
        ...
        <div class="flex flex-col lg:flex-row gap-6 lg:gap-16">
            ...
            <aside class="w-full lg:w-80">
                <x-job-filters :showWorkType="true" :showCategories="false" />
            </aside>

        <!-- Main Content -->
        <div class="flex-grow">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-xs font-black uppercase tracking-[0.3em] text-gray-400">{{ $category->name }} Jobs - {{ $jobs->total() }} Verified Vacancies</h2>
                <form method="GET" class="flex items-center gap-2">
                    @foreach(request()->except('sort') as $key => $value)
                        <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                    @endforeach
                    <span class="text-xs font-bold text-gray-400 uppercase">Sort by:</span>
            ...
            <div class="py-20 text-center">
                ...
                <h3 class="text-lg font-bold text-gray-900">No {{ strtolower($category->name) }} jobs found</h3>
