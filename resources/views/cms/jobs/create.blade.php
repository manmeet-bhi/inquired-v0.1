...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex items-center mb-6 sm:mb-8">
        <a href="{{ route('cms.jobs') }}" class="p-2 -ml-2 text-slate-600 hover:text-slate-900 mr-2 sm:mr-4 transition-colors">
            <i data-lucide="arrow-left" class="w-5 h-5"></i>
        </a>
        <h1 class="text-2xl sm:text-3xl font-bold text-slate-900">Create Job</h1>
    </div>

    <div class="cms-card p-6 lg:p-10 mb-8 overflow-visible">
        <form action="{{ route('cms.jobs.store') }}" method="POST" class="space-y-6">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="cms-form-group">
                    <label for="title" class="cms-label">Job Title <span class="text-rose-500">*</span></label>
                    <input type="text" id="title" name="title" value="{{ old('title') }}" required class="cms-input @error('title') border-red-500 @enderror" placeholder="e.g. Senior Software Engineer">
                    @error('title')
                ...
                <div class="cms-form-group">
                    ...
                    <input type="text" id="job_id" name="job_id" value="{{ old('job_id') }}" class="cms-input @error('job_id') border-red-500 @enderror" placeholder="e.g. JOB-1042, REQ-9821">
                    @error('job_id')
                        <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="cms-form-group">
                    ...
                    <input type="text" id="location" name="location" value="{{ old('location') }}" required class="cms-input @error('location') border-red-500 @enderror" placeholder="e.g. Remote, New York, etc.">
                    @error('location')
                        <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="cms-form-group">
                    ...
                    <select id="type" name="type" required class="cms-select @error('type') border-red-500 @enderror">
                        ...
                        <option value="freelancing" {{ old('type') == 'freelancing' ? 'selected' : '' }}>Freelancing</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                    @enderror
                ...
                <div class="cms-form-group">
                    ...
                    <select id="experience" name="experience" class="cms-select @error('experience') border-red-500 @enderror">
                        ...
                        <option value="Freshers can apply" {{ old('experience') == 'Freshers can apply' ? 'selected' : '' }}>Freshers can apply</option>
                        <option value="0-1 Year" {{ old('experience') == '0-1 Year' ? 'selected' : '' }}>0-1 year</option>
                ...
                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" id="salary_min" name="salary_min" value="{{ old('salary_min') }}" min="0" class="cms-input @error('salary_min') border-red-500 @enderror">
                        @error('salary_min')
                            <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" id="salary_max" name="salary_max" value="{{ old('salary_max') }}" min="0" class="cms-input @error('salary_max') border-red-500 @enderror">
                        @error('salary_max')
                            <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="md:col-span-2 space-y-4">
                    <div class="flex items-center">
                        ...
                        <input type="checkbox" id="has_apply_link" name="has_apply_link" value="1" 
                               {{ old('has_apply_link', '1') == '1' ? 'checked' : '' }} 
                    ...
                    <div id="apply_link_container" class="{{ old('has_apply_link', '1') == '1' ? '' : 'hidden' }}">
                        <label for="application_url" class="cms-label">Application URL</label>
                        <input type="url" id="application_url" name="application_url" value="{{ old('application_url') }}" placeholder="https://example.com/apply" class="cms-input @error('application_url') border-red-500 @enderror">
                ...
                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="cms-card p-5 bg-gradient-to-br from-amber-50/50 to-orange-50/30 border border-amber-200/80 rounded-xl">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <input type="checkbox" id="is_featured" name="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} class="w-5 h-5 text-amber-600 border-slate-300 rounded focus:ring-amber-500 cursor-pointer mt-1">
                        </div>
                    </div>
                    ...
                    <div class="cms-card p-5 bg-slate-50 border border-slate-200 rounded-xl">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', '1') == '1' ? 'checked' : '' }} class="w-5 h-5 text-blue-600 border-slate-300 rounded focus:ring-blue-500 cursor-pointer mt-1">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-8 pt-6 border-t border-slate-100">
                <button type="submit" class="cms-btn cms-btn-primary w-full sm:w-auto">
                    ...
                    <span class="btn-text">Create Job Post</span>
                </button>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        toggleApplyLink(checkbox.checked);
    });
</script>
