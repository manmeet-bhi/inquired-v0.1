...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">Update and manage this blog post</p>
        </div>
    ...
    <form action="{{ route('cms.posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
                ...
                <div class="cms-card p-6">
                    <div class="cms-form-group">
                        ...
                        <textarea name="excerpt" rows="3" 
                                  ...
                                  placeholder="Brief summary of the post appearing in cards, SEO snippets, and social shares...">{{ old('excerpt', $post->excerpt) }}</textarea>
                        <div class="flex items-center justify-between mt-2">
            ...
            <div class="lg:col-span-4 space-y-5 lg:sticky lg:top-8">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden transition-all duration-200">
                    <button type="button" onclick="toggleCollapse('section-publish', this)" class="w-full px-5 py-4 flex items-center justify-between bg-slate-50/80 hover:bg-slate-100/80 transition-colors text-left border-b border-slate-100">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-500">Status, dates & tags</p>
                            </div>
                        </div>
                    ...
                    <div id="section-publish" class="p-5 space-y-4">
                        <!-- Timestamps Info -->
                        <div class="text-xs text-slate-600 bg-slate-50 p-3.5 rounded-xl border border-slate-100 space-y-1.5">
                            <div class="flex items-center justify-between">
                                <span class="text-slate-400">Created</span>
                                <span class="font-semibold text-slate-700">{{ $post->created_at ? $post->created_at->format('M d, Y · h:i A') : 'N/A' }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-slate-400">Updated</span>
                                <span class="font-semibold text-slate-700">{{ $post->updated_at ? $post->updated_at->format('M d, Y · h:i A') : 'N/A' }}</span>
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden transition-all duration-200">
                    ...
                    <div id="section-featured-image" class="p-5 space-y-4">
                        ...
                        <div class="space-y-1.5">
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-semibold text-slate-600 uppercase tracking-wide">Current Image</span>
                                <span class="text-[11px] font-medium text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md">Active</span>
                            </div>
                            <div class="relative group rounded-xl overflow-hidden border border-slate-200 shadow-xs">
                                <img src="{{ $post->featured_image_url }}" alt="{{ $post->title }}" class="w-full h-36 object-cover">
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="relative">
                            ...
                            <div onclick="document.getElementById('featured-image-input').click()" 
                                 class="border-2 border-dashed border-slate-300 rounded-xl p-5 text-center cursor-pointer hover:border-indigo-500 hover:bg-indigo-50/30 transition-all group">
                                <div class="space-y-2">
                                    ...
                                    <p class="text-sm font-semibold text-slate-700 group-hover:text-indigo-600 transition-colors">{{ $post->featured_image ? 'Replace image' : 'Click to upload image' }}</p>
                            ...
                            <div id="image-preview" class="hidden mt-3">
                                <div class="relative group rounded-xl overflow-hidden border border-slate-200 shadow-xs">
                                    <img id="preview-img" class="w-full h-36 object-cover">
