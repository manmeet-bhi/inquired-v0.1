...
@section('title', 'Create Company - CMS')
...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex items-center mb-8">
        ...
        <div>
            <h1 class="text-3xl font-bold text-slate-900">Create Company</h1>
        </div>
    </div>

    <div class="cms-card p-6 lg:p-10 mb-8 overflow-visible">
        <form action="{{ route('cms.companies.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2 cms-form-group">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" required class="cms-input @error('name') border-red-500 @enderror" placeholder="e.g. Acme Corp">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" id="industry" name="industry" value="{{ old('industry') }}" class="cms-input" placeholder="e.g. Technology">
                    @error('industry')
                        <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="type" name="type" class="cms-select">
                        ...
                        <option value="sme" {{ old('type') == 'sme' ? 'selected' : '' }}>SME</option>
                        <option value="mnc" {{ old('type') == 'mnc' ? 'selected' : '' }}>MNC</option>
                        <option value="indian_mnc" {{ old('type') == 'indian_mnc' ? 'selected' : '' }}>Indian MNCs</option>
                        <option value="enterprise" {{ old('type') == 'enterprise' ? 'selected' : '' }}>Enterprise</option>
                        <option value="unicorn" {{ old('type') == 'unicorn' ? 'selected' : '' }}>Unicorn</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" id="founded_year" name="founded_year" value="{{ old('founded_year') }}" min="1800" max="{{ date('Y') }}" class="cms-input" placeholder="e.g. 2010">
                    @error('founded_year')
                        <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="url" id="website" name="website" value="{{ old('website') }}" placeholder="https://example.com" class="cms-input">
                    @error('website')
                        <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="url" id="linkedin_url" name="linkedin_url" value="{{ old('linkedin_url') }}" placeholder="https://linkedin.com/company/..." class="cms-input">
                    @error('linkedin_url')
                        <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="4" class="cms-textarea" placeholder="Describe the company...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
