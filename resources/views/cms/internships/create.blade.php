...
@section('title', 'Create New Internship - CMS')
...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex items-center mb-8">
        ...
        <div>
            <h1 class="text-3xl font-bold text-slate-900">Create New Internship</h1>
        </div>
    </div>

    <div class="cms-card p-6 lg:p-10 mb-8 overflow-visible">
        <form action="{{ route('cms.internships.store') }}" method="POST" class="space-y-6">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="cms-form-group">
                    <label for="title" class="cms-label">Internship Title <span class="text-rose-500">*</span></label>
                    <input type="text" id="title" name="title" value="{{ old('title') }}" placeholder="e.g. Software Development Intern" required class="cms-input @error('title') border-red-500 @enderror">
                    @error('title')
                ...
                <div class="cms-form-group">
                    ...
                    <input type="text" id="job_id" name="job_id" value="{{ old('job_id') }}" placeholder="e.g. INT-1042, REQ-9821" class="cms-input @error('job_id') border-red-500 @enderror">
                    @error('job_id')
                        <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="cms-form-group">
                    ...
                    <input type="text" id="location" name="location" value="{{ old('location') }}" placeholder="e.g. New York, NY" required class="cms-input @error('location') border-red-500 @enderror">
                    @error('location')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="cms-form-group">
                    ...
                    <select id="level" name="level" required class="cms-select @error('level') border-red-500 @enderror">
                        ...
                        <option value="junior" {{ old('level') == 'junior' ? 'selected' : '' }}>Junior Level</option>
                    </select>
                    @error('level')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" id="salary_min" name="salary_min" value="{{ old('salary_min') }}" min="0" placeholder="e.g. 10000" class="cms-input @error('salary_min') border-red-500 @enderror">
                        @error('salary_min')
                            <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" id="salary_max" name="salary_max" value="{{ old('salary_max') }}" min="0" placeholder="e.g. 20000" class="cms-input @error('salary_max') border-red-500 @enderror">
                        @error('salary_max')
                            <p class="mt-1 text-sm text-red-600 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-8 pt-6 border-t border-slate-100">
                <button type="submit" class="cms-btn cms-btn-primary w-full sm:w-auto">
                    ...
                    <span class="btn-text">Create Internship</span>
                </button>
